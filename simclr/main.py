...
if __name__ == '__main__':
    ...
    global_model = Model(feature_dim).cuda()
    flops, params = profile(global_model, inputs=(torch.randn(1, 3, 32, 32).cuda(),))
    flops, params = clever_format([flops, params])
    print('# Model Params: {} FLOPs: {}'.format(params, flops))
    # optimizer = optim.Adam(global_model.parameters(), lr=1e-3, weight_decay=1e-6)
    ...
    for epoch in range(1, args.linear_epochs + 1):
        ...
        data_frame.to_csv('../results/fedsimclr_linear_statistics.csv', index_label='epoch')
